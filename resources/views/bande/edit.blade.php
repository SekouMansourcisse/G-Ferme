            ...
            <div class="content">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        @endif
                        <form method="POST" id="edit-bande-form" action="{{ route('bande.update', $bande->id) }}">
                            @csrf
                            <div class="row">
                                        ...
                                        <select class="form-control" id="responsable" name="responsable" required>
                                            @foreach($users as $user)
                                                @if ($user->profil == "simple utilisateur")
                                                    <option value="{{ $user->id }}" {{ $bande->responsable == $user->id ? 'selected' : '' }}>
                                ...
                                <div class="col-lg-6 col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="nom_bande" name="nom_bande" value="{{ $bande->nom_bande }}" required>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-sm-12">
                                    <div class="form-group">
                                        <label for="cheptel_depart">Cheptel de départ <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" id="cheptel_depart" name="cheptel_depart" value="{{ $bande->cheptel_depart }}" required>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="type_elevage" name="type_elevage" required>
                                            <option value="Poulet de chair" {{ $bande->type_elevage == 'Poulet de chair' ? 'selected' : '' }}>Poulet de chair</option>
                                            <option value="Poules pondeuses" {{ $bande->type_elevage == 'Poules pondeuses' ? 'selected' : '' }}>Poules pondeuses</option>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="souche_bande" id="souche_bande" class="form-control" required>
                                            ...
                                            @foreach($souches as $souche)
                                                <option value="{{ $souche->id }}" {{ $bande->souche_bande == $souche->id ? 'selected' : '' }}>
                                                    {{ $souche->Denomination }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-lg-6 col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <input type="date" class="form-control" id="date_demarrage" name="date_demarrage" value="{{ $bande->date_demarrage }}" required>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-sm-12">
                                    <div class="form-group">
                                        <label for="date_fin">Date de fin prévue <span class="text-danger">*</span></label>
                                        <input type="date" class="form-control" id="date_fin" name="date_fin" value="{{ $bande->date_fin }}" readonly required>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-sm-12">
                                    <div class="form-group">
                                        <label for="age_arrive">Âge d'arrivée de la bande (Jours) <span class="text-danger">*</span></label>
                                        <input type="number" class="form-control" id="age_arrive" name="age_arrive" value="{{ $bande->age_arrive }}" required>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-sm-12">
                                    <div class="form-group">
                                        <label for="poid_moyen_depart">Poids moyen de départ (Grams) <span class="text-danger">*</span></label>
                                        <input type="number" class="form-control" id="poid_moyen_depart" name="poid_moyen_depart" value="{{ $bande->poid_moyen_depart }}" required>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-sm-12">
                                    <div class="form-group">
                                        <label for="cout_acquisition">Coût de l'acquisition <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" id="cout_acquisition" name="cout_acquisition" value="{{ $bande->cout_acquisition }}" required readonly>
                                    </div>
                                </div>
                                ...
                                <div class="table-responsive mt-4">
                                    <table class="table">
                                        ...
                                        <tbody id="produits-table-body">
                                            ...
                                            @php
                                                // Initialiser le nombre de cheptel
                                                $nombreCheptel = '';

                                                // Vérifiez si le poulailler est dans bandePoulaillers et extraire le nombre de cheptel
                                                foreach ($bandePoulaillers as $bandePoulailler) {
                                                    list($id, $cheptel) = explode('*', $bandePoulailler);
                                                    if ($id == $poulailler->id) {
                                                        $nombreCheptel = $cheptel; // assigner le nombre de cheptel si trouvé
                                                        break;
                                                    }
                                                }
                                            @endphp
                                            <tr>
                                                <td>
                                                    <input type="checkbox" name="poulailler_selectionne[]" value="{{ $poulailler->id }}"
                                                    {{ $nombreCheptel ? 'checked' : '' }}>
                                                </td>
                                                <td><input type="text" name="denomination[]" value="{{ $poulailler->Denomination }}" class="form-control" readonly></td>
                                                <td><input type="number" name="contenance[]" value="{{ $poulailler->contenance_normale }}" class="form-control" readonly></td>
                                                <td><input type="text" name="dimension[]" value="{{ $poulailler->Dimension }}" class="form-control" readonly></td>
                                                <td><input type="number" name="nombre_cheptel[]" value="{{ $nombreCheptel }}" class="form-control"></td>
                                            </tr>
                                            @endforeach
                                        </tbody>
