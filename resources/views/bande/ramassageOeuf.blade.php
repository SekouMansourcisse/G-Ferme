        ...
        <div class="col-md-12">
            <div class="page-header">
                <div class="page-title" id="titre">
                    <h4>Ramassages d'Oeufs</h4>
                    <h6>Liste Ramassages</h6>
                </div>
                @can('create operation sur bande')
                @if ($bande->etat==1)
                <div class="page-btn">
                    <button id="ajoutBtn" class="btn btn-added"><img src="{{ asset('assets/img/icons/plus.svg')}}" alt="img">Démarrer le Ramassage
                        </button>
                </div>
                @endif
                @endcan
            ...
            <div class="card">
                ...
                <div class="card-body" id="ramassageOeufList">
                    <div class="table-top">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach($ramassageOeufs as $ramassage)
                                    @php
                                        $poulaillerData = explode(',', $ramassage->poulailler);
                                        $totalQuantite = $ramassage->Total;
                                        $tauxPointe = $ramassage->taux_pointe;
                                    @endphp

                                    @foreach($poulaillerData as $poulaillerInfo)
                                        @php
                                            list($poulaillerId, $qte1, $qte2, $qte3) = explode('*', $poulaillerInfo);
                                            $poulaillerName = App\Models\Poulailler::find($poulaillerId)->Denomination;
                                            $qteRamassee = $qte1 + $qte2 + $qte3;
                                        @endphp
                                        <tr>
                                            @if ($loop->first)
                                            ...
                                            <td>{{ $qte2 }}</td>
                                            <td>{{ $qte3 }}</td>
                                            <td>{{ $qteRamassee }} oeufs</td>
                                            @if ($loop->first)
                                                <td rowspan="{{ count($poulaillerData) }}">{{ $totalQuantite }} oeufs</td>
                                                <td rowspan="{{ count($poulaillerData) }}">{{ $tauxPointe }} %</td>
                                                <td rowspan="{{ count($poulaillerData) }}">{{ $ramassage->commentaire }}</td>
                                                @can('edit operation sur bande')
            ...
            <div class="card">
             <div class="card-body" id="ajoutForm" style="display:none;">

                <form action="{{ route('ramassageoeufs.store') }}" method="POST">
                    @csrf
                    <div class="row">
                        ...
                        <div class="col-lg-6 col-sm-8 col-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="Date" name="Date" value="{{ \Carbon\Carbon::now()->format('Y-m-d') }}" required readonly>
                            </div>
                        </div>
                        <div class="col-lg-6 col-sm-8 col-12">
                            <div class="form-group">
                                <label for="typeR" class="form-label">Type Ramassage</label>
                                <select name="typeR" class="form-control" required>
                                    <option value="1">RamassageN°1</option>
                                    <option value="2">RamassageN°2</option>
                                    <option value="3">RamassageN°3</option>
                                </select>
                            </div>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    @foreach(explode(',', $bande->poulailler) as $poulailler)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" class="form-control" name="qte[{{ $id }}]" required>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                    ...
                    <div class="row">
                        <div class="col-lg-8">
                            ...
                            <button type="button" id="cancelajoutBtn" class="btn btn-secondary" onclick="window.history.back();">Annuler</button>
                        </div>
                    </div>
                </form>
...
<script>
    document.getElementById('ajoutBtn').addEventListener('click', function() {
        ...
        document.getElementById('ramassageOeufList').style.display = 'none';
        document.getElementById('ajoutForm').style.display = 'block';
        document.getElementById('ajoutBtn').style.display = 'none';
    ...
    document.getElementById('cancelajoutBtn').addEventListener('click', function() {
        // Cache le formulaire et affiche la liste
        document.getElementById('ajoutForm').style.display = 'none';
        document.getElementById('ramassageOeufList').style.display = 'block';
