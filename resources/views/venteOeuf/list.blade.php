            ...
            <div class="content">
                <div class="page-header">
                    <div class="page-title">
                        <h4>Liste des ventes d'Oeufs</h4>
                        <h6>visualiser vos ventes en Oeufs</h6>
                    </div>
                    @can('create Ventes Oeufs')
                        <div class="page-btn">
                            <a href="{{ url('vente-oeufs/create') }}" class="btn btn-added"><img
                                ...
                                <ul>
                                    <li>
                                        <a data-bs-toggle="tooltip" data-bs-placement="top"
                                            href="{{ url('vente_oeuf/export-pdf') }}" title="pdf"><img
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @foreach ($ventes as $vente)
                                        <tr>
                                            <td>{{ $vente->Date_op }}</td>
                                            <td>Type de client: {{ $vente->type_client }} <br>
                                                Nom&Prenom:{{ $vente->NomPrenomClient }} <br> N°Tel: {{ $vente->phone }}
                                            </td>
                                            <td>
                                                @foreach (explode(',', $vente->infosOeuf) as $oeuf)
                                                    @php
                                                        $oeufData = explode('*', $oeuf);
                                                        if (count($oeufData) === 2) {
                                                            [$id, $qte] = $oeufData;
                                                            $oeufNom =
                                                                App\Models\CategorieOeuf::find($id)?->Denomination ??
                                                                'Inconnu';
                                                        } else {
                                                            $oeufNom = 'Format invalide';
                                                            $qte = 0;
                                                        }
                                                    @endphp
                                                    {{ $oeufNom }} ({{ $qte }} plateaux)<br>
                                                @endforeach
                                            ...
                                            @can('edit Ventes Oeufs')
                                                <td>
                                                    ...
                                                    <a class="me-3 edit-item-btn"
                                                        href="{{ route('vente-oeufs.edit', $vente->id) }}">
                                                        <img src="{{ asset('assets/img/icons/edit.svg') }}" alt="img">
                                                    </a>


                                                    @can('delete Ventes Oeufs')
